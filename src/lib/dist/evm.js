...
var integralSdk = require('@cryptoalgebra/integral-sdk');
...
// evm/constants/v3.ts
var V2_FEE_PATH_PLACEHOLDER = '0000000';
...
function computePairAddress(token0, token1) {
  const salt = viem.keccak256(
    viem.encodePacked(["address", "address", "bool"], [token0.address, token1.address, false])
  );
...
var computeV3PoolAddress = memoize__default.default(
  integralSdk.computePoolAddress,
...
function encodeMixedRouteToPath(route, exactOutput) {
  ...
  const { path, types } = route.pools.reduce(
    ({ inputToken, path: path2, types: types2 }, pool, index) => {
      ...
      return {
        inputToken: outputToken,
        types: !isV3Pool(pool) ? [...types2, "uint24", "address"] : [...types2, "uint24", "address"],
        path: !isV3Pool(pool) ? [...path2, 0, outputToken.address] : [...path2, fee, outputToken.address]
      };
    },
...
function parsePool(chainId, pool) {
  ...
  if (pool.type === 2 /* STABLE */) {
    return {
      ...
      fee: new sdk.Percent(parseFloat(pool.fee) * 1e6, ONE_HUNDRED * BigInt(1e6))
    };
  }
...
var getV3PoolsWithoutTicksOnChain = createOnChainPoolFactory({
  abi: algebraPoolABI,
  getPossiblePoolMetas: ([currencyA, currencyB]) => {
    return [v3Sdk.FeeAmount.LOWEST].map((fee) => ({
...
var getV3PoolMeta = memoize__default.default(
  ([currencyA, currencyB, feeAmount]) => ({
    address: integralSdk.computePoolAddress({
      tokenA: currencyA.wrapped,
      tokenB: currencyB.wrapped,
      poolDeployer: ALGEBRA_POOL_DEPLOYER,
      initCodeHashManualOverride: POOL_INIT_CODE_HASH
...
var getV3PoolMetas = memoize__default.default(
  (pair) => [v3Sdk.FeeAmount.LOWEST].map((fee) => getV3PoolMeta([...pair, fee])),
  ([currencyA, currencyB]) => {
    if (currencyA.wrapped.equals(currencyB.wrapped)) {
...
function createGetV2Quote(isExactIn = true) {
  return function getV2Quote({ reserve0, reserve1 }, amount) {
    sdk.Pair.getAddress = computePairAddress;
    ...
    const createGetRoutesWithQuotes = (isExactIn = true) => {
      ...
      return async function getRoutesWithQuote(routes, { blockNumber: blockNumberFromConfig, gasModel, retry: retryOptions, signal }) {
        ...
        async function getQuotes({ gasLimitPerCall }) {
          ...
          console.log('ASDD QUOTER', inputs, quoterAddress)
          ...
          try {
            ...
            console.log('MIXED QUOTES', quoterAddress, results)
...
function createQuoteProvider(config) {
  ...
  const createGetRouteWithQuotes = (isExactIn = true) => {
    ...
    return async function getRoutesWithQuotes(routes, { blockNumber, gasModel, signal }) {
      ...
      const a = getMixedRouteQuotes(mixedRoutesHaveV3Pool, { blockNumber, gasModel, retry: { retries: 0 }, signal })

      a.then(v => console.log('MIXED QQQQQ', v)).catch(e => console.log('MIXED ERRR', e))
      
      throw new Error('FINSIH')
...
var publicClient = viem.createPublicClient({
  ...
  transport: viem.http("https://holesky.drpc.org"),
  batch: {
    multicall: {
      batchSize: 1024 * 200
    }
...
var v3SubgraphClient = new graphqlRequest.GraphQLClient("https://api.thegraph.com/subgraphs/name/iliaazhel/integral-core");
var v2SubgraphClient = new graphqlRequest.GraphQLClient("https://api.thegraph.com/subgraphs/name/camelotlabs/camelot-amm");
...
var ZERO8 = BigInt(0);
var REFUND_ETH_PRICE_IMPACT_THRESHOLD = new sdk.Percent(BigInt(50), BigInt(100));
var _SwapRouter = class {
  ...
  static encodeV2Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    ...
    const recipient = routerMustCustody ? ADDRESS_THIS : typeof options.recipient === "undefined" ? MSG_SENDER : sdk.validateAndParseAddress(options.recipient);
    if (trade.tradeType === sdk.TradeType.EXACT_INPUT) {
          ...
          if (mixedRouteIsAllV3(newRoute)) {
            ...
            if (isExactIn) {
              if (trade.inputAmount.currency.isNative) {
                ...
                calldatas.push(viem.encodeFunctionData({
                  abi: _SwapRouter.ABI,
                  functionName: "swapExactETHForTokensSupportingFeeOnTransferTokens",
                  args: exactInputParams
                }));
                ...
                const exactInputParams = [amountIn, performAggregatedSlippageCheck ? BigInt(0) : amountOut, path, recipient];
                calldatas.push(viem.encodeFunctionData({ abi: _SwapRouter.ABI, functionName: "swapExactTokensForETHSupportingFeeOnTransferTokens", args: exactInputParams }));
              } else {
                const exactInputParams = [amountIn, performAggregatedSlippageCheck ? BigInt(0) : amountOut, path, recipient];
                calldatas.push(viem.encodeFunctionData({ abi: _SwapRouter.ABI, functionName: "swapExactTokensForTokensSupportingFeeOnTransferTokens", args: exactInputParams }));
              }
            } else {
...
Object.defineProperty(exports, 'CurrencyAmount', {
  enumerable: true,
  get: function () { return sdk.CurrencyAmount; }
});
Object.defineProperty(exports, 'Native', {
  enumerable: true,
  get: function () { return sdk.Native; }
});
Object.defineProperty(exports, 'Percent', {
  enumerable: true,
  get: function () { return sdk.Percent; }
});
